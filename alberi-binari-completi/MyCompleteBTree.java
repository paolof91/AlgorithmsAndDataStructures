...
public class MyCompleteBTree<E> implements CompleteBTree<E> {
	protected ArrayList<Position<E>> list;
	
	public MyCompleteBTree() {
	...
	public boolean isInternal(Position<E> p) {
		int index = ((MyCompleteBPosition<E>)p).index();
		return 2*index<=size();
	}
	
	public boolean isRoot(Position<E> p) {
		return ((MyCompleteBPosition<E>)p).index() == 1;
	}
	...
	public Position<E> replace(Position<E> p,E element) {
		int index = ((MyCompleteBPosition<E>)p).index();
		Position<E> n = new MyCompleteBPosition<E>(element,index); 
		list.set(index,n);
		return n;
	}
	...
	public boolean hasLeft(Position<E> p) {
		return 2*((MyCompleteBPosition<E>)p).index()<=size();
	}
	
	public boolean hasRight(Position<E> p) {
		return 2*((MyCompleteBPosition<E>)p).index()+1<=size();
	}
	
	public Position<E> add(E element) {	
		Position<E> n = new MyCompleteBPosition(element,size()+1);
		list.add(n);
		return n;
	}
